...
function createBlueMissles(){
   ...
   bluemissle.src = "realblue.png";
   
   missles.push({image:bluemissle, x:main_x, y:main_y});
   return bluemissle;
...
function createBrownMissles(){
    ...
    brownmissle.src = "red.png";
    
    missles2.push({image:brownmissle, x: main_x2, y: main_y2});
    return brownmissle;
}

function createBossMissles(){
    bossmiss = new Image;
    bossmiss.src = "bossmiss.png";
    
    bossmissles.push({image:bossmiss, x: 50, y: 0});
    return bossmiss;
}

function createBossMissles2(){
    bossmiss2 = new Image;
    bossmiss2.src = "bossmiss.png";
    
    bossmissles2.push({image:bossmiss2, x: 30, y: -25});
    return bossmiss2;
}
...
function gameLoop(){
  ...
  window.location.href = "https://static-michaelnowlin5.c9users.io/project/project.html";
};
...
function spawnEnemy(){
    ...
    enemyi.src = "enemy.png";
    
 var c = Math.random() * game.height - game.height/2;
   enemys.push({image:enemyi, x: 50, y:c});
...
function spawnBoss(){
    ...
    bosses.push({image:boss, x: 50, y:0});
}
...
for(var o = 0; o <bosses.length; o++){
   ...
   for(var c = 0; c < missles.length; c++){
            if(Math.abs(missles2[c].x - bosses[o].x) <= 30 && missles2[c].y - bosses[o].y <= 40 &&  missles2[c].y - bosses[o].y >= -1){
                  bossHp -= 1;
                  missles2.splice(c,1);
            }
}
    ...
    for(var e = 0; e < bossmissles2.length; e++){
        if(Math.abs(main_x - bossmissles2[e].x)  <= 16 && main_y - bossmissles2[e].y <= 13 &&  main_y - bossmissles2[e].y >= -1){
